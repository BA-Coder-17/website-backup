...
class DTCoreShortcodesDefination {
	...
	function dt_sc_tabs($attrs, $content = null) {
		...
		if($style == 'default')
			...
			foreach ( $tab_titles as $tab ) {
				...
				} elseif( isset($tab_atts['icon_type']) && $tab_atts['icon_type'] === 'image' ){

					$icon = isset( $tab_atts['image'] ) ? $tab_atts['image'] : '';
					
					//image
					$each_tab_image = wpb_getImageBySize( array( 'attach_id' => $icon, 'thumb_size' => 'full' ));
					$each_tab_image_url = $each_tab_image['p_img_large'][0];
					$each_tab_image = $each_tab_image['thumbnail'];
									
					$icon = '<div class="dt-sc-tab-image-holder">'.$each_tab_image.'</div>';
					
				}

				if(isset($tab_atts['icon_type']) && $tab_atts['icon_type'] != 'image' ){
	...
	function dt_sc_button($attrs, $content = null) {
		...
		if(function_exists('vc_build_link')) {
			...
			if(isset($link['http']) && !empty($link['http'])) {
				$a_href = $link['http'];
			} else if(isset($link['https']) && !empty($link['https'])) {
				$a_href = $link['https'];
			} else if(isset($link['url']) && !empty($link['url'])) {
				$a_href = $link['url'];
			}
			$a_title = $link['title'];
	...
	function dt_sc_blockquote($attrs, $content = null) {
		...
		$class_str = join( '', array_unique( $class_str ) );

		$out = "<blockquote class='{$type} {$class_str} {$class}' {$style}>{$content}{$cite}</blockquote>";
		return $out;
	}
	...
	function dt_sc_simple_heading( $attrs, $content = null ){
		...
		if( $type == 'simple' ) {
			...
			$out .= "<{$extra_text_tag}>{$extra_text}</{$extra_text_tag}>";
			$out .= "</div>";
		} elseif( $type == 'title-with-image' ) {
			$subheading_color_str = '';
			if($subheading_color != '') {
				$subheading_color_str = 'style="color:'.$subheading_color.'"';
			}

			$image_url = wpb_getImageBySize( array( 'attach_id' => $image, 'thumb_size' => 'full' ));
			$image_url = '<span>'.$image_url['thumbnail'].'</span>';

			$out = '<div class="dt-sc-title with-image '.esc_attr($class).'">';
			if($image_position == 'top') {
				$out .= $image_url;
			}
			$out .= "<{$tag}>{$text}";
				if($subtext != '') {
					$out .= "<strong {$subheading_color_str}>{$subtext}</strong>";
				}
			$out .= "</{$tag}>";
			if($image_position == 'bottom') {
				$out .= $image_url;
			}	
			$out .= '<h6>'.DTCoreShortcodesDefination::dtShortcodeHelper ( $content ).'</h6>';		
			$out .= "</div>";
		}
		return $out;
	}
	...
	function dt_sc_twitter_tweets($attrs, $content = null) {
		...
		if($username && $consumerkey && $consumersecret && $accesstoken && $accesstokensecret) {
			...
			require_once(KALVI_CORE_PLUGIN."/apis/twitteroauth/twitteroauth.php");
				...
				$out .= '<article id="post-'.$id.'" class="'.esc_attr($post_classes).'">';
					switch( $style ):
						...
						case 'entry-date-left':
							ob_start();
							$obj->kalvi_post_date_left_style( $id, $meta );
							$out .= ob_get_clean();
						break;

						case 'entry-date-author-left':
							ob_start();
							$obj->kalvi_post_date_author_left_style( $id, $meta );
							$out .= ob_get_clean();
						break;
						
						case 'entry-date-left outer-frame-border':
							ob_start();
							$obj->kalvi_post_date_left_framed_border_style( $id, $meta );
							$out .= ob_get_clean();
						break;

						case 'blog-modern-style':
							ob_start();
							$obj->kalvi_post_modern_style( $id, $meta );
							$out .= ob_get_clean();
						break;

						case 'bordered':
							ob_start();
							$obj->kalvi_post_bordered_style( $id, $meta );
							$out .= ob_get_clean();
						break;

						case 'classic':
							ob_start();
							$obj->kalvi_post_classic_style( $id, $meta );
							$out .= ob_get_clean();
						break;
						
						case 'entry-overlay-style':
							ob_start();
							$obj->kalvi_post_overlay_style( $id, $meta );
							$out .= ob_get_clean();
						break;
						
						case 'overlap':
							ob_start();
							$obj->kalvi_post_overlap_style( $id, $meta );
							$out .= ob_get_clean();
						break;
						
						case 'entry-center-align':
							ob_start();
							$obj->kalvi_post_stripe_style( $id, $meta );
							$out .= ob_get_clean();
						break;
						
						case 'entry-fashion-style':
							ob_start();
							$obj->kalvi_post_fashion_style( $id, $meta );
							$out .= ob_get_clean();
						break;
						
						case 'entry-minimal-bordered':
							ob_start();
							$obj->kalvi_post_minimal_bordered_style( $id, $meta );
							$out .= ob_get_clean();
						break;
						...
						default:
							ob_start();
							$obj->kalvi_post_default_style( $id, $meta );
							$out .= ob_get_clean();
						break;
	...
	function dt_sc_portfolios( $attrs, $content = null ){
		...
		$img_size = array( 
			'1' => 'full',
			'2' => 'kalvi-portfolio-ii-column',
			'3' => 'kalvi-portfolio-iii-column',
			'4' => 'kalvi-portfolio-iv-column'
		);
	...
	function dt_sc_events_list( $attrs, $content = null ){
		...
		$out = '<div class="dt-sc-events-list-wrapper">';
			...
			if(!empty($title))	$out .= "<h3>{$title}</h3>";
				...
				$events = tribe_get_events( array( 'posts_per_page' => $limit,
	...
	function dt_sc_special_events_list( $attrs, $content = null ){
		...
		if(class_exists('Tribe__Events__Main')):
			...
			$events = tribe_get_events( array( 'posts_per_page' => $limit, 'tax_query'=> array( array( 'taxonomy' => 'tribe_events_cat', 'field' => 'term_id', 'terms' => $cats ) ) ));
			if($events): $i = 1;
				...
				switch($type):
					...
					default:
						foreach($events as $post):
							...
							$out .= '<div class="'.$column_class.' '.$temp_class.'">';
					...
					case 'type6':
						foreach($events as $post):
							...
							$out .= '<div class="'.$column_class.' '.$temp_class.'">';
								$out .= '<div class="dt-sc-event type6">';
									 $out .= '<div class="dt-sc-event-thumb">';
										if(has_post_thumbnail()):
											$attr = array('title' => get_the_title(), 'alt' => get_the_title());
											$out .= get_the_post_thumbnail($post->ID, 'designthemes-event-list2', $attr);
									 ...
									 $out .= '</div>';
									 $out .= '<div class="dt-sc-event-meta">';
									    $out .= '<p class="dt-sc-event-date"><span>'.tribe_get_start_date ( $event_id, true, 'd' ).'</span>'.tribe_get_start_date ( $event_id, true, 'M' ).'</p>';
									 	$out .= '<h2 class="entry-title"><a href="'.get_permalink().'">'.get_the_title().'</a></h2>';
									 $out .= '</div>';
								$out .= '</div>';
							$out .= '</div>';
			...
			$out .= '<p>'.esc_html__('Kindly install & activate The Events Calendar plugin and check it.', 'designthemes-core').'</p>';
	...
	function dt_sc_down_count($attrs, $content = null) {
		...
		$out .= '<div class="downcount '.$class.'" data-date="'.kalvi_cs_get_option('comingsoon-launchdate', date('m/d/Y')).'" data-offset="'.kalvi_cs_get_option('comingsoon-timezone', '0').'">';
