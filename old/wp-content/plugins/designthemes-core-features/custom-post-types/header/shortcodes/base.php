...
if (! class_exists ( 'DTBaseSC' ) ) {
    ...
    class DTBaseSC {
        ...
        function dt_print_css( $css ) { 
            if( !empty( $css ) ) {
                wp_enqueue_style( 'kalvi-custom-inline' );
                wp_add_inline_style( 'kalvi-custom-inline', $css );
            }
        }
        ...
        function dt_current_skin( $code = 'primary-color' ) {
            ...
            $color = '';
            $mode = get_theme_mod( 'use-predefined-skin', kalvi_defaults('use-predefined-skin') );

            if( $mode ) {
                $skin = get_theme_mod( 'predefined-skin', kalvi_defaults('predefined-skin' ) );
                $skin = kalvi_skins( $skin );
                $color = $skin[$code];
            } else {
                $color = get_theme_mod( $code, kalvi_defaults( $code ) );
            }
