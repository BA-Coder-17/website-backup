...
<div id="header-wrapper" class="<?php echo esc_attr($header_class); ?>">
    ...
    <header id="header">
        <div class="container"><?php
            /**
              * spalab_header hook.
              * 
              * @hooked spalab_vc_header_template - 10
              *
              */
            do_action( 'spalab_header' ); ?>
      </div>
    </header><!-- **Header - End ** -->
    ...
    <?php
        if( !empty( $global_breadcrumb ) ) {
            ...
            $bstyle = spalab_cs_get_option( 'breadcrumb-style', 'default' );
            $style = spalab_breadcrumb_css();

            $title = get_the_archive_title();
            $breadcrumbs[] = __('Portfolios','designthemes-core');
            $breadcrumbs[] = '<a href="'. get_category_link( get_query_var('portfolio_tags') ) .'">' . single_tag_title('', false) . '</a>';

            spalab_breadcrumb_output ( '<h1>'.$title.'</h1>', $breadcrumbs, $bstyle, $style );
        }
    ?><!-- ** Breadcrumb End ** -->                
...
<div id="main">
    ...
    <div class="container"><?php
        ...
        $layout = spalab_page_layout( $page_layout );
        extract( $layout );
        ...
        <section id="primary" class="<?php echo esc_attr( $page_layout );?>"><?php
            ...
            $design_hover_type = cs_get_option( 'gallery-archives-hover-design-type' );
            ...
            if( have_posts() ) :
                $i = 1;?>
                <div class="dt-sc-portfolio-container <?php echo esc_attr($allow_space);?> <?php echo esc_attr( $design_hover_type );?> <?php echo esc_attr($post_style);?>">
									...
									<div class="image-overlay">
										...
										<?php elseif( $post_style == "type1" || $post_style == "type2" || $post_style == "type5" || $post_style == "type7" || $post_style == "type8"):?>
											...
											<div class="image-overlay-details"><?php
												if( $post_style != "type2"):
													if( $post_style == "type7" ):
														the_terms( $post->ID, 'portfolio_tags', "<p class='tags'>",' ','</p>');
													else:
														the_terms( $post->ID, 'portfolio_tags', "<p class='tags'>",', ','</p>');
													endif;
												endif;?>
                                        ...
                                        <?php elseif($post_style == "type10" || $post_style == "type11" || $post_style == "type12" || $post_style == "type13" || $post_style == "type14" ):?>
                                            <div class="portfolio-detail">
                                            <?php if(function_exists('likeThis')):?>
                                                    <div class="views"><i class="fa fa-heart-o"> </i><?php echo generateLikeString($id, isset($_COOKIE["like_" . $the_id])); ?></div>
                                            <?php endif;?>
                                            
                                            <div class="portfolio-meta-content">
                                            <h5><a title="<?php the_title();?>" href="<?php the_permalink();?>"><?php the_title(); ?></a></h5>
                                            <p><?php echo esc_html($portfolio_item_meta['sub-title']); ?></p>
                                            </div> 
                                        </div>
                                        <?php endif;?>
									</div>
