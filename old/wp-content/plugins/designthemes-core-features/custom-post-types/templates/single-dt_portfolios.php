        ...
        if ( $show_sidebar ) {
            if ( $show_left_sidebar ) {
                ...
                <section id="secondary-left" class="secondary-sidebar <?php echo esc_attr( $sidebar_class.$sticky_class );?>"><?php
                	 kalvi_show_sidebar( 'dt_portfolios', $post->ID, 'left' ); ?>
                ...
                while( have_posts() ) {
                    ...
                    <article id="post-<?php the_ID();?>" <?php post_class(array('dt-portfolio-single'));?>><?php
							...
							if( array_key_exists("portfolio-video",$settings) && $settings['portfolio-video']!='') {
								...
								echo '<li>';
									if (strpos($portfolio_video_url, "vimeo")) :
										...
										echo "<iframe src='http".kalvi_ssl()."://player.vimeo.com/video/{$url}' width='1170' height='700' frameborder='0'></iframe>";
									elseif(strpos($portfolio_video_url, "?v=")):
										$url = substr( strrchr($portfolio_video_url, "="),1);
										echo "<iframe src='http".kalvi_ssl()."://www.youtube.com/embed/{$url}?wmode=opaque'  width='1170' height='700' frameborder='0'></iframe>";
									endif;
								echo '</li>';
								}
							}
							...
							if( array_key_exists("portfolio-video",$settings) && $settings["portfolio-video"]!="" ) {
								$portfolio_video_final = array();
									...
									foreach ( $portfolio_video_final as $portfolio_video_url ) {
										if (strpos($portfolio_video_url, "vimeo")) :
											$url = substr( strrchr($portfolio_video_url, "/"),1);
											echo "<a class='video' data-slide-index='".$j."' href=''><img src='".kalvi_getVimeoThumb($url)."' alt=".__('Vimeo Video','designthemes-core')." /></a>";													
										elseif(strpos($portfolio_video_url, "?v=")):
											$url = substr( strrchr($portfolio_video_url, "="),1);
											echo "<a class='video' data-slide-index='".$j."' href=''><img src='http".kalvi_ssl()."://img.youtube.com/vi/{$url}/0.jpg' alt=".__('Youtube Video','designthemes-core')." /></a>";
										endif;
                        ...
                        <div class="dt-portfolio-single-details">
                            ...
                            <?php if( array_key_exists('portfolio_opt_flds', $settings) ): ?>
                            ...
                            <?php else: ?>
                                ...
                                <div class="column dt-sc-one-column first">
                                    ...
                                    <?php the_content(); ?>
									<?php echo get_the_term_list($post->ID, 'portfolio_entries', '<p class="portfolio-categories"> <i class="pe-icon pe-network"> </i> ', ', ', '</p> '); ?>
									<?php echo get_the_term_list($post->ID, 'portfolio_tags', '<p class="portfolio-tags"> <i class="pe-icon pe-ticket"> </i> ', ', ', '</p> '); ?>
                                </div>
        ...
        if ( $show_sidebar ) {
            if ( $show_right_sidebar ) {
                ...
                <section id="secondary-right" class="secondary-sidebar <?php echo esc_attr( $sidebar_class.$sticky_class );?>"><?php
                	 kalvi_show_sidebar( 'dt_portfolios', $post->ID, 'right' ); ?>
