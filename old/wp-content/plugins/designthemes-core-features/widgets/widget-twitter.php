<?php
class Kalvi_Twitter extends WP_Widget {
	#1.constructor
	function __construct() {
		$widget_options = array("classname"=>'widget_tweetbox', 'description'=>esc_html__('To show latest twitter tweets', 'kalvi'));
		parent::__construct(false,KALVI_THEME_NAME.esc_html__(' Twitter Widget','kalvi'),$widget_options);
	}

	#2.widget input form in back-end
	function form($instance) {
		$instance = wp_parse_args( (array) $instance,array( 'title' => esc_html__('Latest Tweets','kalvi'), 'count' => '3', 'username' => '',
        ...
        <p><label for="<?php echo esc_attr($this->get_field_id('title')); ?>"><?php esc_html_e('Title:','kalvi');?> 
        ...
        <p><label for="<?php echo esc_attr($this->get_field_id('consumer_key')); ?>"><?php esc_html_e('Consumer Key:','kalvi');?> 
        ...
        <p><label for="<?php echo esc_attr($this->get_field_id('consumer_secret')); ?>"><?php esc_html_e('Consumer Secret:','kalvi');?> 
        ...
        <p><label for="<?php echo esc_attr($this->get_field_id('access_token')); ?>"><?php esc_html_e('Access Token:','kalvi');?> 
        ...
        <p><label for="<?php echo esc_attr($this->get_field_id('access_token_secret')); ?>"><?php esc_html_e('Access Token Secret:','kalvi');?> 
        ...
        <p><label for="<?php echo esc_attr($this->get_field_id('username')); ?>"><?php esc_html_e('Enter your twitter username:','kalvi');?>
        ...
        <p><label for="<?php echo esc_attr($this->get_field_id('count')); ?>"><?php esc_html_e('How many entries do you want to show:','kalvi');?>
        ...
        <p><input type="checkbox"  id="<?php echo esc_attr($this->get_field_id('exclude_replies'));?>" name="<?php echo esc_attr($this->get_field_name('exclude_replies'));?>" 
			<?php checked($exclude_replies); ?> /> <label for="<?php echo esc_attr($this->get_field_id('exclude_replies')); ?>"><?php esc_html_e('Exclude @replies','kalvi');?></label></p>
            
        <p><input type="checkbox"  id="<?php echo esc_attr($this->get_field_id('time'));?>" name="<?php echo esc_attr($this->get_field_name('time'));?>" 
			<?php checked($time); ?> /> <label for="<?php echo esc_attr($this->get_field_id('time')); ?>"><?php esc_html_e('Show time of tweet','kalvi');?></label></p>
        ...
        <p><input type="checkbox"  id="<?php echo esc_attr($this->get_field_id('display_avatar'));?>" name="<?php echo esc_attr($this->get_field_name('display_avatar'));?>" 
				<?php checked($display_avatar); ?> /> <label for="<?php echo esc_attr($this->get_field_id('display_avatar')); ?>"><?php esc_html_e('Show user avatar','kalvi');?></label></p>		
<?php
	}
	...
	function widget($args, $instance) {
		extract($args);
			...
			echo kalvi_before_after_widget( $before_widget );

			if ( !empty( $title ) ) {
				echo kalvi_widget_title( $before_title . $title . $after_title );	
			}
			...
			if($username && $consumer_key && $consumer_secret && $access_token && $access_token_secret && $count) { 
				...
				if($twitter && is_array($twitter)) {
					foreach( $twitter as $tweet ):
						...
						echo '<li>';
								if( $display_avatar )
									echo '<span class="tweet-thumb"><a href="http://twitter.com/'.$username.'" title="'.esc_attr__( 'Username', 'kalvi' ).'"><img src="'.esc_url($tweet->user->profile_image_url).'" alt="'.esc_attr__( 'Image', 'kalvi' ).'" /></a></span>';
					...
					echo '<li>'.esc_html__('No public Tweets found','kalvi').'</li>';
				}
				echo "</ul>";
			}

			echo kalvi_before_after_widget( $after_widget ); 
	}
}?>