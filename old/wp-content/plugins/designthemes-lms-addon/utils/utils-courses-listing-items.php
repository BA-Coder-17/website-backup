	...
	if(!$purchased_paid_course && !$free_course) {
		...
		if(dtlms_check_item_is_in_cart($course_id)) {
			...
			$output .= '<div class="dtlms-coursedetail-cart-details">';
				$output .= '<a href="'.esc_url(wc_get_cart_url()).'" target="_self" class="dtlms-coursedetail-cart-link dtlms-button small filled"><i class="fa fa-cart-plus"></i>'.esc_html__('View Cart','dtlms').'</a>';
			$output .= '</div>';
