...
class CSFramework_Option_Gallery extends CSFramework_Options {
  ...
  public function output(){
    ...
    $add    = ( ! empty( $this->field['add_title'] ) ) ? $this->field['add_title'] : esc_html__( 'Add Gallery', 'kalvi' );
    $edit   = ( ! empty( $this->field['edit_title'] ) ) ? $this->field['edit_title'] : esc_html__( 'Edit Gallery', 'kalvi' );
    $clear  = ( ! empty( $this->field['clear_title'] ) ) ? $this->field['clear_title'] : esc_html__( 'Clear', 'kalvi' );
    ...
    if( ! empty( $value ) ) {
      ...
      foreach ( $values as $id ) {
        ...
        echo '<li><img src="'. $attachment[0] .'" alt="'.esc_attr__( 'Image', 'kalvi' ).'" /></li>';
      }
