...
if ( ! class_exists( 'Gutenberg_Editor_CSS' ) ) :
	...
	class Gutenberg_Editor_CSS {
		...
		function __construct() {
			...
			add_action( 'current_screen', array( $this,  'kalvi_current_screen_hook' ) );

			if ( class_exists( 'Classic_Editor' ) ) {
				add_filter( 'tiny_mce_before_init', array( $this, 'kalvi_editor_dynamic_styles' ) );
			}
			
			add_action( 'enqueue_block_editor_assets', array( $this, 'kalvi_backend_editor_styles' ) );

		}

		public function kalvi_current_screen_hook( $current_screen ) {

			if ( 'post' == $current_screen->base ) {

				$urls = KALVI_Kirki::enqueue_fonts_url();
		...
		public function kalvi_editor_dynamic_styles( $mceInit ) {
			...
			$styles .= $this->kalvi_backend_classic_editor_styles();
		...
		public function kalvi_backend_classic_editor_styles() {
			...
			$status_of_body_content_typo = get_theme_mod( 'customize-body-content-typo', kalvi_defaults('customize-body-content-typo') );
	
			if( $status_of_body_content_typo == '1' ) {
	
				$font = get_theme_mod('body-content-typo', kalvi_defaults('body-content-typo'));
			...
			$body_bg_color = get_theme_mod('body-bg-color', kalvi_defaults('body-bg-color'));
			$body_content_color = get_theme_mod('body-content-color', kalvi_defaults('body-content-color'));
			$body_a_color = get_theme_mod('body-a-color', kalvi_defaults('body-a-color'));
			$body_a_hover_color = get_theme_mod('body-a-hover-color', kalvi_defaults('body-a-hover-color'));
			...
			for ($i = 1; $i <= 6; $i++) :
				$status_of_h_typo = get_theme_mod( 'customize-body-h'.$i.'-typo', kalvi_defaults('customize-body-h'.$i.'-typo') );
				if($status_of_h_typo == 1){				
					$font = get_theme_mod('h'.$i.'',kalvi_defaults('h'.$i.''));
					$font_family    = isset($font['font-family']) ? $font['font-family'] : '';
					$font_size    	= isset($font['font-size']) ? $font['font-size'] : '';
		...
		public function kalvi_backend_editor_styles() {
			...
			wp_enqueue_style( 'kalvi-gutenberg', get_theme_file_uri('/css/admin-gutenberg.css'), false, KALVI_THEME_VERSION, 'all' );
			wp_add_inline_style( 'kalvi-gutenberg', $styles  );
