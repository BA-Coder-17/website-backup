 ...
 * Theme support
 * --------------------------------------------------------------------------- */
if (!function_exists('kalvi_features')) {

	function kalvi_features() {
		...
		if ( function_exists( 'add_theme_support' ) ) {
			...
			add_image_size( 'kalvi-blog-thumb', 150, 120, true  ); 	// blog - list
			add_image_size( 'kalvi-blog-ii-column', 750, 500, true  ); 	// blog - ii column
			add_image_size( 'kalvi-blog-iii-column', 540, 360, true  ); 	// blog - iii column
			add_image_size( 'kalvi-blog-list', 600, 400, true  ); 	// blog - list
			add_image_size( 'kalvi-blog-ii-column-masonry', 750 ); 	// blog - ii column masonry
			add_image_size( 'kalvi-blog-iii-column-masonry', 540 ); 	// blog - iii column masonry

			add_image_size( 'kalvi-portfolio-ii-column', 960, 720, true  ); 	// portfolio - ii column
			add_image_size( 'kalvi-portfolio-iii-column', 640, 480, true  ); 	// portfolio - iii column
			add_image_size( 'kalvi-portfolio-iv-column', 510, 383, true  ); 	// portfolio - iv column
			add_image_size( 'kalvi-portfolio-iii&iv-fullwidth', 780, 585, true  ); 	// portfolio - iii&iv column

			add_image_size( 'kalvi-event-list', 420, 336, true  ); 	// event-calendar - list
			add_image_size( 'kalvi-event-single-type1', 812, 545, true  ); // event-calendar - single
			add_image_size( 'kalvi-event-single-type4', 603, 487, true  ); // event-calendar - single
			add_image_size( 'kalvi-event-single-type5', 746, 770, true  ); // event-calendar - single
			add_image_size( 'kalvi-event-list2', 420, 275, true  ); 	// event-calendar - shortcode list
		}
		...
		register_nav_menus( array(
			'main-menu' => esc_html__('Main Menu', 'kalvi'),
		) );
		...
		add_theme_support( 'editor-color-palette', array(
			array(
				'name'  => esc_html__( 'Primary Color', 'kalvi' ),
				'slug'  => 'primary',
				'color' => $primary_color,
			),
			array(
				'name'  => esc_html__( 'Secondary Color', 'kalvi' ),
				'slug'  => 'secondary',
				'color' => $secondary_color,
			),
			array(
				'name'  => esc_html__( 'Tertiary Color', 'kalvi' ),
				'slug'  => 'tertiary',
				'color' => $tertiary_color,
			)
		));
	}
	add_action('after_setup_theme', 'kalvi_features');
}

/* ---------------------------------------------------------------------------
 * Check Activate Theme
 * --------------------------------------------------------------------------- */
if (!function_exists('kalvi_activation_function')) {
	function kalvi_activation_function( $old_name, $old_theme ) {
		...
		$options = array (
		  ...
		  'post-archives-readmore' => '[dt_sc_button title="'.esc_attr__('Read More', 'kalvi').'" style="filled" icon_type="fontawesome" iconalign="icon-right with-icon" iconclass="fa fa-long-arrow-right" class="type1"]',
		  'post-style' => 'blog-default-style',
		  'post-foramt-meta' => false,
		  ...
		  'breadcrumb-delimiter' => 'fa fa-arrow-circle-right',
		...
		if(!in_array($old_name, array('Kalvi', 'Kalvi Child'))) {
			update_option( '_cs_options', $options );
		}
	}
	add_action("after_switch_theme", "kalvi_activation_function", 10, 2 );
}

/* ---------------------------------------------------------------------------
 *	Under Construction
 * --------------------------------------------------------------------------- */
if( ! function_exists('kalvi_under_construction') ){
	function kalvi_under_construction(){
...
if( cs_get_option( 'enable-comingsoon' ) ):
	add_action('template_redirect', 'kalvi_under_construction', 30);

	// getting shortcode css ----------------------
	add_action('wp_enqueue_scripts', 'kalvi_rand_css', 101);
	function kalvi_rand_css() {
		$id = cs_get_option( 'comingsoon-pageid' );
		if ( $id ) {
			$shortcodes_custom_css = get_post_meta( $id, '_wpb_shortcodes_custom_css', true );
			if ( ! empty( $shortcodes_custom_css ) ) {
				
				wp_register_style( 'vc-shortcodes-custom-'.$id, '', false, KALVI_THEME_VERSION, 'all' );	
 ...
 * --------------------------------------------------------------------------- */
if( !function_exists('kalvi_wp_list_categories') ){
	function kalvi_wp_list_categories( $output ){
	...
	add_filter('wp_list_categories', 'kalvi_wp_list_categories');
}

/* ---------------------------------------------------------------------------
 * Filter to modify default list archive widget view
 * --------------------------------------------------------------------------- */
if( !function_exists('kalvi_wp_list_archive') ){
	function kalvi_wp_list_archive( $link_html,$url, $text, $format, $before, $after ) {
	...
	add_filter('get_archives_link', 'kalvi_wp_list_archive', 10, 6);	
}

/* ---------------------------------------------------------------------------
 * Filter to execute shortcode inside contact form7
 * --------------------------------------------------------------------------- */
if( !function_exists('kalvi_wpcf7_form_elements') ){
	function kalvi_wpcf7_form_elements($form) {
		$form = do_shortcode( $form );
		return $form;
	}
	add_filter('wpcf7_form_elements', 'kalvi_wpcf7_form_elements');
}

/* ---------------------------------------------------------------------------
 * Pagination for Blog and Portfolio
 * --------------------------------------------------------------------------- */
function kalvi_pagination( $query = false, $load_more = false ){
	global $wp_query;
		...
		if( $load_more ){
			...
			if( $paged < $total ){
				$output .= '<div class="column one pager_wrapper pager_lm">';
					$output .= '<a class="pager_load_more button button_js" href="'. get_pagenum_link( $next ) .'">';
						...
						$output .= '<span class="button_label">'. esc_html__('Load more', 'kalvi') .'</span>';
					$output .= '</a>';
				$output .= '</div>';
			}
			...
			$output .= '<div class="column one pager_wrapper">';
				...
				$args = array(
					...
					'mid_size'           => $mid_size,
					'prev_next'          => true,
					'prev_text'          => '<i class="fa fa-angle-double-left"></i>'.esc_html__('Prev', 'kalvi'),
					'next_text'          => esc_html__('Next', 'kalvi').'<i class="fa fa-angle-double-right"></i>',
					'type'               => 'list'
				);
...
function kalvi_events_title() {
	...
	if( tribe_is_month() && !is_tax() ) { 
		$title = sprintf( esc_html__( 'Events for %s', 'kalvi' ), date_i18n( 'F Y', strtotime( tribe_get_month_view_date() ) ) );
	} elseif( class_exists('Tribe__Events__Pro__Main') && tribe_is_week() )  {
		$title = sprintf( esc_html__('Events for week of %s', 'kalvi'), date_i18n( $date_format, strtotime( tribe_get_first_week_day($wp_query->get('start_date') ) ) ) );
	} elseif( class_exists('Tribe__Events__Pro__Main') && tribe_is_day() ) {
		$title = esc_html__( 'Events for', 'kalvi' ) . ' ' . date_i18n( $date_format, strtotime( $wp_query->get('start_date') ) );
	} elseif( class_exists('Tribe__Events__Pro__Main') && (tribe_is_map() || tribe_is_photo()) ) {
		if( tribe_is_past() ) {
			$title = esc_html__( 'Past Events', 'kalvi' );
		} else {
			$title = esc_html__( 'Upcoming Events', 'kalvi' );
		}
		...
		$title = esc_html__('Tag Archives','kalvi');
	}
	return $title;
}
 ...
 * Excerpt
 * --------------------------------------------------------------------------- */
function kalvi_excerpt($limit = NULL) {
 ...
 * --------------------------------------------------------------------------- */
function kalvi_wp_kses($content) {
 ...
 * --------------------------------------------------------------------------- */
if(!function_exists('kalvi_hex2rgb')) {

	function kalvi_hex2rgb($hex) {
 ...
 * Theme Comment Style
 * --------------------------------------------------------------------------- */
function kalvi_comment_style( $comment, $args, $depth ) {
 ...
 * --------------------------------------------------------------------------- */
function kalvi_get_page_permalink_by_its_template( $template ) {
	$permalink = '#';
 ...
 * Theme show sidebar
 * --------------------------------------------------------------------------- */
function kalvi_show_sidebar( $type, $id, $position = 'right' ) {
 ...
 * --------------------------------------------------------------------------- */
function kalvi_global_variables($variable = '') {
 ...
 * break oembeds.
 * --------------------------------------------------------------------------- */
function kalvi_undo_recurrence_oembed_logic() {
...
add_action( 'init', 'kalvi_undo_recurrence_oembed_logic' );

/* ---------------------------------------------------------------------------
 * Walker Page for kalvi_new_wp_page_menu
 * --------------------------------------------------------------------------- */
class KALVI_Walker_Page extends Walker_Page {
 ...
 * --------------------------------------------------------------------------- */
add_filter('upload_mimes', 'kalvi_upload_mimes');
function kalvi_upload_mimes( $existing_mimes = array() ){
...
*/
function kalvi_before_after_widget ( $content ) {
...
*/
function kalvi_widget_title( $content ) {
