	...
	<div class="entry-details">
	  ...
	  <div class="entry-meta"><?php
		  ...
		  <div class="entry-title">
              <h4><a href="<?php the_permalink();?>" title="<?php printf( esc_attr__('Permalink to %s','kalvi'), the_title_attribute('echo=0'));?>"><?php the_title(); ?></a></h4>
          </div>

		  <div class="entry-info">
			  <div class="author <?php echo esc_attr( $show_author_meta );?>"><i class="zmdi zmdi-account-box zmdi-hc-fw"> </i> <a href="<?php echo get_author_posts_url(get_the_author_meta('ID'));?>" title="<?php esc_attr_e('View all posts by ', 'kalvi'); echo get_the_author();?>"><?php echo get_the_author();?></a></div>
			  ...
			  <div class="comments <?php echo esc_attr($show_comment_meta);?>"><?php
				  comments_popup_link('<i class="zmdi zmdi-comment-list zmdi-hc-fw"> </i> 0 '.esc_html__('comments', 'kalvi'), '<i class="zmdi zmdi-comment-list zmdi-hc-fw"> </i> 1 '.esc_html__('comment', 'kalvi'), '<i class="zmdi zmdi-comment-list zmdi-hc-fw"> </i> % '.esc_html__('comments', 'kalvi'), '', '<i class="zmdi zmdi-comment-list zmdi-hc-fw"> </i>'.esc_html__('comments off', 'kalvi'));?></div>

			  <?php if( shortcode_exists( 'dt_sc_post_view_count' ) && class_exists( 'DTCorePlugin' ) ): ?>
				  <div class="views <?php echo esc_attr($show_post_views);?>"><i class="zmdi zmdi-eye zmdi-hc-fw"> </i> <?php echo do_shortcode('[dt_sc_post_view_count post_id="'.$post->ID.'" /]');?> <?php esc_html_e('Views', 'kalvi');?> </div>
              <?php endif;?>
			  ...
			  <?php if( shortcode_exists( 'dt_sc_post_like_count' ) && class_exists( 'DTCorePlugin' ) ): ?>
                  <div class="likes dt_like_btn <?php echo esc_attr($show_post_likes);?>">
                      ...
                      <a href="#" data-postid="<?php the_ID(); ?>" data-action="like">
                          ...
                          <span><?php esc_html_e('Likes', 'kalvi');?></span>
                      </a>
                  </div>
              <?php endif;?>
		  </div>
	  ...
	  <div class="entry-meta-data">
		 <?php the_tags("<p class='tags {$show_tag_meta}'><span>".esc_html__('Tag List', 'kalvi')."</span> ",' ',"</p>"); ?>
	  </div><!-- Category & Tag -->
	  ...
	  <?php edit_post_link( esc_html__( ' Edit ','kalvi' ) ); ?>
	</div><!-- .entry-details -->