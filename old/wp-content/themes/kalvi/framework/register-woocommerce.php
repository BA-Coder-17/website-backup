...
// Change Image Sizes --------------------------------------------------------
	$pagenow = kalvi_global_variables('pagenow');
	if ( is_admin() && isset( $_GET['activated'] ) && $pagenow == 'themes.php' )
		add_action( 'init', 'kalvi_woo_image_dimensions', 1 );

	function kalvi_woo_image_dimensions() {
...
// To Remove Page wrapper Start
	...
	add_action( 'woocommerce_before_main_content', 'kalvi_woo_output_content_wrapper', 11 );
	function kalvi_woo_output_content_wrapper() {
		...
		echo '		<div class="container">';
             			do_action( 'kalvi_header', $shop_page_id );
		echo '		</div>';
		echo '	</header>';
		...
		if( !empty( $global_breadcrumb ) ) {
			...
			if( is_shop() ) {
				...
				if( isset( $settings['enable-sub-title'] ) && $settings['enable-sub-title'] ) {
					...
					$style = kalvi_breadcrumb_css( $bcsettings );

					kalvi_breadcrumb_output ( '<h1>'.$title.'</h1>', $breadcrumbs, $bstyle, $style );
                }
			}
			...
			if( is_product_category() || is_product_tag() ) {
                ...
                kalvi_breadcrumb_output ( '<h1>'.single_term_title( '', false ).'</h1>', $breadcrumbs, $bstyle, $style );
			}			
		}
			...
			if ( $show_sidebar ) {
				if ( $show_left_sidebar ) {
					...
					echo '<section id="secondary-left" class="secondary-sidebar '.$sidebar_class.$sticky_class.'">';
						kalvi_show_sidebar( 'page', $shop_page_id, 'left' );
					echo '</section>';
				}
			}
		}
...
// To Remove Page wrapper End
	...
	add_action( 'woocommerce_after_main_content', 'kalvi_woo_output_content_wrapper_end', 11 );
	function kalvi_woo_output_content_wrapper_end() {
			...
			if ( $show_sidebar ) {
				if ( $show_right_sidebar ) {
					...
					echo '<section id="secondary-right" class="secondary-sidebar'.$sidebar_class.$sticky_class.'">';
						kalvi_show_sidebar( 'page', $shop_page_id, 'right' );
					echo '</section>';					
				}
			}
		}
...
// Shop Column
	add_filter( 'loop_shop_per_page', 'kalvi_woo_posts_per_page' );
	add_filter( 'loop_shop_columns', 'kalvi_woo_loop_columns' );

	// No.of products per page --------------------------------------------------
	function kalvi_woo_posts_per_page( $count ) {
	...
	function kalvi_woo_loop_columns( $columns ) {
...
// Remove Yith Buttons
kalvi_woo_remove_anonymous_object_action('woocommerce_after_shop_loop_item', 'YITH_WCQV_Frontend', 'yith_add_quick_view_button' , 15 );
kalvi_woo_remove_anonymous_object_action('woocommerce_after_shop_loop_item', 'YITH_Woocompare_Frontend', 'add_compare_link' , 20 );

// Product Category
	add_action( 'woocommerce_before_subcategory', 'kalvi_woo_product_style_start', 5 );
	add_action( 'woocommerce_after_subcategory', 'kalvi_woo_product_style_end', 10 );

	add_action( 'woocommerce_before_subcategory_title', 'kalvi_woo_before_subcategory_title', 5 );
	remove_action('woocommerce_shop_loop_subcategory_title', 'woocommerce_template_loop_category_title', 10 );
	add_action( 'woocommerce_after_subcategory_title', 'kalvi_woo_after_subcategory_title', 10 );

// Product Wrapper
	add_action( 'woocommerce_before_shop_loop_item', 'kalvi_woo_product_style_start', 1 );
	add_action( 'woocommerce_after_shop_loop_item', 'kalvi_woo_product_style_end', 100 );
	...
	add_action( 'woocommerce_after_shop_loop_item', 'kalvi_woo_shop_overview_show_price', 10 );

// To Remove Pagination
	remove_action( 'woocommerce_after_shop_loop', 'woocommerce_pagination', 10 );
	add_action( 'woocommerce_after_shop_loop', 'kalvi_woo_after_shop_loop', 10 );

// Before shop loop item -----------------------------------------------------
function kalvi_woo_product_style_start() {
...
function kalvi_woo_product_style_end() {
...
// After shop loop item -----------------------------------------------------
function kalvi_woo_shop_overview_show_price() {
	global $product;
	$output = "";
	...
	$output .= "<div class='product-thumb'>";
		...
		if( $product->is_on_sale() and $product->is_in_stock() )
			$output .= '<span class="onsale"><span>'.esc_html__('Sale','kalvi').'</span></span>';

		elseif(!$product->is_in_stock())
			$output .= '<span class="out-of-stock"><span>'.esc_html__('Out of Stock','kalvi').'</span></span>';

		if( $product->is_featured())
			$output .= '<div class="featured-tag"><div><i class="fa fa-thumb-tack"></i><span>'.esc_html__('Featured','kalvi').'</span></div></div>';

		$output .= '<a class="image" href="'.get_permalink().'" title="'.the_title_attribute('echo=0').'">';
		$id = $product->get_id();
...
// After shop loop ----------------------------------------------------------
function kalvi_woo_after_shop_loop() {
	echo '<div class="pagination">';
		if( function_exists( 'kalvi_pagination' ) )
			echo kalvi_pagination();
		else
...
function kalvi_woo_before_subcategory_title() {
	echo '<div class="product-thumb"><span class="image">';
}

// After Category Title ----------------------------------------------------
function kalvi_woo_after_subcategory_title( $category ) {
	echo '</span></div>';
...
// Single Product 
	...
	add_action( 'woocommerce_after_single_product_summary', 'kalvi_woo_show_upsell', 16 );

	// Related Products
	remove_action( 'woocommerce_after_single_product_summary', 'woocommerce_output_related_products', 20);
	add_action( 'woocommerce_after_single_product_summary', 'kalvi_woo_show_related', 21 );

	// Sale Flash
	remove_action( 'woocommerce_before_single_product_summary', 'woocommerce_show_product_sale_flash', 10 );
	add_action( 'woocommerce_before_single_product_summary','kalvi_woo_show_product_wrapper',10 );
	add_action( 'woocommerce_after_single_product_summary','kalvi_woo_close_product_wrapper',10 );

	add_action( 'woocommerce_product_thumbnails', 'kalvi_woo_show_product_sale_flash', 30 );

	/* --------------------------------------------------------------------------
	 * Single Product
	 * Showing Upsell Products
	 * -------------------------------------------------------------------------- */
	function kalvi_woo_show_upsell() {
		global $woocommerce_loop;
	 ...
	 * -------------------------------------------------------------------------- */
	function kalvi_woo_show_related() {
		global $woocommerce_loop;
	 ...
	 * -------------------------------------------------------------------------- */
	function kalvi_woo_show_product_wrapper() {
	 ...
	 * -------------------------------------------------------------------------- */
	function kalvi_woo_close_product_wrapper() {
		echo '</div>';
	}
	 ...
	 * Product Sale Flash
	 * -------------------------------------------------------------------------- */
	function kalvi_woo_show_product_sale_flash() {
		global $product;

		$out = '<div class="product-status-labels">';
		if( $product->is_on_sale() and $product->is_in_stock() )
			$out .= '<span class="onsale"><span>'.esc_html__('Sale!','kalvi').'</span></span>';

		elseif(!$product->is_in_stock())
			$out .= '<span class="out-of-stock">'.esc_html__('Out of Stock','kalvi').'</span>';

		if($product->is_featured())
			$out .= '<div class="featured-tag"><div><i class="fa fa-thumb-tack"></i><span>'.esc_html__('Featured','kalvi').'</span></div></div>';
...
function kalvi_woo_remove_anonymous_object_action( $tag, $class, $method, $priority = null ){
