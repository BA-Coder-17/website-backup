        ...
        if ( $show_sidebar ) {
            if ( $show_left_sidebar ) {
                ...
                <section id="secondary-left" class="secondary-sidebar <?php echo esc_attr( $sidebar_class.$sticky_class );?>"><?php
                    kalvi_show_sidebar( 'page', $globalid, 'left' ); ?>
        ...
        <section id="primary" class="<?php echo esc_attr( $page_layout );?>"><?php
        	...
        	<div class="dt-sc-posts-list-wrapper"><?php
				...
				if( $the_query->have_posts() ):
					...
					$meta = $obj->kalvi_post_meta_fields(false, $globalid);
							...
							<article id="post-<?php the_ID();?>" <?php post_class('blog-entry '.$post_style.' '.'format-'.$format);?>><?php
								...
								switch( $post_style ):
									...
									case 'entry-date-left':
										$obj->kalvi_post_date_left_style( $post_ID, $meta );
									break;

									case 'entry-date-author-left':
										$obj->kalvi_post_date_author_left_style( $post_ID, $meta );
									break;

									case 'entry-date-left outer-frame-border':
										$obj->kalvi_post_date_left_framed_border_style( $post_ID, $meta );
									break;

									case 'blog-modern-style':
										$obj->kalvi_post_modern_style( $post_ID, $meta );
									break;

									case 'bordered':
										$obj->kalvi_post_bordered_style( $post_ID, $meta );
									break;

									case 'classic':
										$obj->kalvi_post_classic_style( $post_ID, $meta );
									break;

									case 'entry-overlay-style':
										$obj->kalvi_post_overlay_style( $post_ID, $meta );
									break;

									case 'overlap':
										$obj->kalvi_post_overlap_style( $post_ID, $meta );
									break;

									case 'entry-center-align':
										$obj->kalvi_post_stripe_style( $post_ID, $meta );
									break;

									case 'entry-fashion-style':
										$obj->kalvi_post_fashion_style( $post_ID, $meta );
									break;

									case 'entry-minimal-bordered':
										$obj->kalvi_post_minimal_bordered_style( $post_ID, $meta );
									break;
									...
									default:
										$obj->kalvi_post_default_style( $post_ID, $meta );
									break;
					...
					<h2><?php esc_html_e('Nothing Found.', 'kalvi'); ?></h2>
					<p><?php esc_html_e('Apologies, but no results were found for the requested archive.', 'kalvi'); ?></p><?php
				endif;?>
    		</div>

			<!-- **Pagination** -->
	        <div class="pagination blog-pagination"><?php echo kalvi_pagination($the_query); ?></div><!-- **Pagination** -->
            <!-- Blog Template Ends -->
        ...
        if ( $show_sidebar ) {
            if ( $show_right_sidebar ) {
                ...
                <section id="secondary-right" class="secondary-sidebar <?php echo esc_attr( $sidebar_class.$sticky_class );?>"><?php
                    kalvi_show_sidebar( 'page', $globalid, 'right' ); ?>
